...
| 项 | 值 |
|----|-----|
| 底层平台 | `WebGL`（需 Switch Platform 到 WebGL） |
| 脚本宏 | `WEIXINMINIGAME`（打包时自动启用） |
| 快捷菜单 | `TEngine → Build → 一键打包微信小游戏` |
| AB 输出 | `Builds/WebGL/DefaultPackage/{版本}/` |
| WebGL 输出 | `Build/WeChatMiniGame/webgl/` |
| 小游戏输出 | 由微信 SDK 导出到 `MiniGameConfig.asset` 配置的 DST 目录 |

**前置条件：**

1. 安装 [微信小游戏转换 SDK](https://github.com/wechat-miniprogram/minigame-tuanjie-transform-sdk.git)  
   `Window → Package Manager → + → Add package from git URL`
2. Switch Platform 到 **WebGL**
3. 执行 `HybridCLR → Generate → All`（首次）
4. 配置 `UpdateSetting.asset`：
   - `playMode` = **WebPlayMode**
   - `ResDownLoadPath` = 资源 CDN 地址（小游戏资源走远程加载）
5. 在微信 SDK 面板（`微信小游戏 → 转换小游戏`）配置 AppID、CDN、导出路径等
...
|--------|--------|
| 目标平台 | 微信小游戏 |
| 内置文件拷贝 | ClearAndCopyAll |
| 最小包模式 | **开启**（减小首包，资源走 CDN） |
| 构建后导出微信小游戏 | 勾选（需已安装 SDK） |
| 由微信 SDK 构建 WebGL | 勾选（推荐，由 SDK 统一处理 WebGL 构建与转换） |

**打包流程：**

```
1. TEngine → Build → 打包工具窗口 → 选择「微信小游戏」
2. 资源版本号 → 点「自动」
3. 确认 WEIXINMINIGAME 宏已启用（窗口内可一键启用）
4. 点「一键构建 (AB + Player)」
   → 编译热更 DLL → 构建 AB → 调用微信 SDK 导出 minigame
5. 用微信开发者工具打开导出的 minigame 目录预览/上传
```

**运行时说明：**

- `ResourceModule` 在 `UNITY_WEBGL && WEIXINMINIGAME` 下自动使用 YooAsset `WechatFileSystem`
- 缓存根目录：`{USER_DATA_PATH}/__GAME_FILE_CACHE`
- 热更 DLL 目录：`HybridCLRData/HotUpdateDlls/WebGL/`
...
| 菜单 | 作用 |
|------|------|
| `TEngine/Build/微信小游戏/启用 WEIXINMINIGAME 宏` | 为 WebGL 平台添加脚本宏 |
| `TEngine/Build/微信小游戏/应用推荐 WebGL 设置` | 压缩禁用、内存 256MB 等 |
| `TEngine/Build/微信小游戏/打开 SDK 安装说明` | 跳转官方文档 |

**未安装 SDK 时：**

仍可完成 AB + WebGL 构建，之后手动执行 `微信小游戏 → 转换小游戏` 完成导出。
...
| `TEngine/Build/一键打包微信小游戏` | — | 微信小游戏完整包 |
